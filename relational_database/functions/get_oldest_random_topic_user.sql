...
BEGIN
    ...
    WHERE 
        q.chat_mode = mode
        AND q.has_topics = FALSE 
        AND q.inserted_at <= NOW() - INTERVAL '10 seconds'        -- Gore matching: true can match with true/null, false can match with false/null, null can match with any
        AND (
            (get_oldest_random_topic_user.gore IS NULL) OR 
            (q.gore IS NULL) OR 
            (get_oldest_random_topic_user.gore = q.gore)
        )
        -- Nudity matching: true can match with true/null, false can match with false/null, null can match with any
        AND (
            (get_oldest_random_topic_user.nudity IS NULL) OR 
            (q.nudity IS NULL) OR 
            (get_oldest_random_topic_user.nudity = q.nudity)
        )
